...
import { mockContent, mockPublications } from '../../mock';
import { CONTENT_STATE_LABELS, PLATFORM_LABELS } from '../../constants';
import { PlatformIcon } from '../../components/ui/PlatformIcon';
import type { ContentState } from '../../types';

const PORTAL_CLIENT_ID = '1';
const CLIENT_NAME = 'Clínica Smile';

type PortalState = 'pending' | 'changes' | 'approved' | 'published';

function classifyState(state: ContentState): PortalState | null {
...
export default function PortalDashboard() {
  ...
  const pieces = mockContent.filter(cp => cp.client_id === PORTAL_CLIENT_ID);
  ...
  const upcomingPubs = publications
  ...
  return (
    <div className="px-4 pt-5 pb-4 space-y-5">
      ...
      <div>
        ...
        <h1 className="text-xl font-bold text-slate-800">{CLIENT_NAME} 👋</h1>
      </div>
      ...
      <div className="grid grid-cols-4 gap-2">
        ...
        <StatChip
          ...
          onClick={() => navigate('/fplus/portal/cronopost')}
        />
        <StatChip
        ...
        <StatChip
          ...
          onClick={() => navigate('/fplus/portal/multimedia')}
        />
      </div>
      ...
      {upcomingPubs.length > 0 && (
        <section>
          <div className="flex items-center justify-between mb-3">
            ...
            <button
              onClick={() => navigate('/fplus/portal/calendar')}
              ...
              return (
                <div key={pub.id} className="flex items-center gap-3 bg-white border border-slate-100 rounded-xl p-3">
                  ...
                  <div className="text-right shrink-0">
                    <p className="text-xs font-medium text-slate-700 capitalize">{dayLabel}</p>
          ...
          <div className="space-y-2">
            {approved.slice(0, 2).map(cp => (
              <div
                ...
                className="flex items-center gap-3 bg-white border border-slate-100 rounded-xl p-3 active:bg-slate-50 cursor-pointer"
              >
...
function StatChip({
  icon, label, count, colorClass, onClick,
}: {
