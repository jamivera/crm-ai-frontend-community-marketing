...
| D-20 | ISSUE-001 (login 500) | Aceptar la 0010 como fix | 0010 omitía el claim si era null, pero no resolvía colisiones futuras para usuarios con client_id | Evidencia de colisiones con claims reservados de GoTrue | **Descartar 0010 y buscar solución definitiva** |
| D-21 | Claim namespacing | Mantener claim `client_id` original | Riesgo persistente de colisión en GoTrue para portal cliente y futuros flujos de OAuth/SSO | Validado login exitoso con namespacing | **Namespacing de client_id a fplus_client_id** (ADR-012, migr. 0011) |
